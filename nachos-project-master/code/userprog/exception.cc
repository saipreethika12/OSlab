...
void handle_SC_Multiply() {
    DEBUG(dbgSys, "Multiply " << kernel->machine->ReadRegister(4) << " + "
                         << kernel->machine->ReadRegister(5) << "\n");
    ...
    DEBUG(dbgSys, "Multiply returning with " << result << "\n");
    /* Prepare Result */
    kernel->machine->WriteRegister(2, (int)result);
...
void handle_SC_Exec() {
    ...
    virtAddr = kernel->machine->ReadRegister(
        4);  // doc dia chi ten chuong trinh tu thanh ghi r
     int character;
     character=kernel->machine->ReadRegister(5);
    ...
    kernel->machine->WriteRegister(2, SysExec(name, character));
