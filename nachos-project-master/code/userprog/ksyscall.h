...
void SysHalt() {time_t current=time(NULL);
	printf("run time %d", current-(kernel->currentThread->starttime));kernel->interrupt->Halt() ; }

int SysAdd(int op1, int op2) { return op1 + op2; }

int SysMultiply(int op1, int op2) { return op1 * op2; }

void SysSleep(int time) { kernel->currentThread->sleepTicksLeft=time;kernel->sleepList->Append(kernel->currentThread); }
